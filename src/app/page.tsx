...
export default function Home() {
  ...
  const saveSession = (score: number, understood: string[], struggled: string[]) => {
    const record: SessionRecord = {
      id: crypto.randomUUID(), subject,
      topic: topic || 'General', difficulty,
      date: Date.now(), score, understood, struggled,
    }
  ...
  const getOpeningLine = (): string => {
    const openings = [
      ...
      `Okay so I'm panicking a bit. ${topic || SUBJECT_LABELS[subject]} is NOT clicking for me. Can you explain it simply? 😅`,
    ...
    if (quickMode) {
      return `Okay, speed mode! ⚡ I'm short on time. Teach me the most important things about ${topic || SUBJECT_LABELS[subject]} that I NEED to remember!`
    }
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          message: text, subject, topic: topic || undefined,
      ...
      setMessages(prev => [...prev, {
        id: crypto.randomUUID(), role: 'classmate',
  ...
  const endSession = async () => {
    setEvaluating(true)
    setStep('report')
    const conversation = messages.map(m => `${m.role === 'user' ? 'Student' : 'Classmate'}: ${m.content}`).join('\n\n')
    try {
      ...
      setEvaluation(d)
      saveSession(d.overallScore || 0, d.understood || [], d.struggled || [])
    } catch {
      setEvaluation({ understood: [], struggled: [], weakSpots: ['Could not deeply analyze this session'], misconceptions: [], overallScore: 0, suggestions: ['Try a longer session for detailed feedback'] })
    } finally { setEvaluating(false) }
  }

  const resetSession = () => { setStep('setup'); setMessages([]); setEvaluation(null); setInput(''); setShowWarning(false); setWarningText('') }

  const continueTeaching = () => {
    setStep('chat'); setEvaluation(null)
    setMessages([{ id: crypto.randomUUID(), role: 'classmate', content: `Hey! I still need more help with ${SUBJECT_LABELS[subject]}${topic ? `, especially ${topic}` : ''}. Can we go again? 🙏`, timestamp: Date.now() }])
  }

  const retryWarning = () => { setShowWarning(false); setWarningText(''); setIsLoading(false) }
      ...
      <main className="min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-lg animate-slideUp">
          <div className="text-center mb-10">
            ...
            <p className="text-[var(--text-secondary)] text-sm md:text-base max-w-sm mx-auto leading-relaxed">Your AI classmate is confused.<br />Teach them what you know — and discover what you don't.</p>
          </div>
          <div className="mb-6">
          ...
          <div className="mb-6">
            <label className="text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider mb-3 block">What topic are you studying? <span className="text-xs font-normal normal-case opacity-60">(optional)</span></label>
                ...
                <div className="mt-3 glass rounded-xl p-4 max-h-48 overflow-y-auto text-left">
                  {sessions.map(s => (
                    <div key={s.id} className="flex items-center justify-between py-2 border-b border-[var(--border)] last:border-0 text-xs">
                      <div><span className="font-medium">{SUBJECT_EMOJIS[s.subject]} {s.topic}</span><span className="text-[var(--text-secondary)] ml-2">— {new Date(s.date).toLocaleDateString()}</span></div>
          ...
          {sessions.length === 0 && <p className="mt-6 text-xs text-center text-[var(--text-secondary)]">No sessions yet. Complete your first session to start tracking your progress.</p>}
        </div>
      </main>
    )
  }
  ...
  if (step === 'chat') {
    return (
      <main className="min-h-screen flex flex-col">
        <header className="glass sticky top-0 z-10 px-4 py-3 flex items-center justify-between border-b border-[var(--border)]">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#6366f1] to-[#818cf8] flex items-center justify-center text-sm flex-shrink-0">{SUBJECT_EMOJIS[subject]}</div>
            <div className="min-w-0">
              <div className="text-sm font-semibold truncate">Your Classmate</div>
              <div className="text-xs text-[var(--text-secondary)] truncate">{SUBJECT_LABELS[subject]}{topic ? ` — ${topic}` : ''}</div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={endSession}
              ...
              style={{ background: 'rgba(248, 113, 113, 0.12)', color: '#f87171', border: '1px solid rgba(248, 113, 113, 0.2)' }}>End</button>
          </div>
        </header>
        <div className="px-4 py-2 text-center border-b border-[var(--border)] bg-[var(--bg-card)]/50">
          <span className="text-xs text-[var(--text-secondary)]">{isLoading ? '🤔 Your classmate is thinking...' : messages.length <= 1 ? '😵‍💫 Your classmate is confused. Start teaching!' : '🧠 Your classmate is learning from you'}</span>
        </div>
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 max-w-2xl mx-auto w-full">
          {messages.map(msg => (
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fadeIn`}>
              <div className={`max-w-[88%] md:max-w-[75%] px-4 py-3 text-sm leading-relaxed ${msg.role === 'user' ? 'bubble-user' : 'bubble-classmate'}`}>{msg.content}</div>
            </div>
          ))}
          {isLoading && (
            <div className="flex justify-start animate-fadeIn">
              <div className="bubble-classmate px-4 py-3 flex gap-1.5"><span className="typing-dot" /><span className="typing-dot" /><span className="typing-dot" /></div>
            </div>
          )}
          {showWarning && (
        ...
        <div className="glass px-4 py-4 max-w-2xl mx-auto w-full rounded-t-2xl border-t border-[var(--border)]">
          <div className="flex gap-3 items-end">
            ...
            <button onClick={() => { if (!showWarning) sendMessage() }}
              ...
              style={{ background: input.trim() && !isLoading && !showWarning ? 'linear-gradient(135deg, #6366f1, #818cf8)' : 'var(--bg-input)', color: input.trim() && !isLoading && !showWarning ? 'white' : 'var(--text-secondary)' }}>Send</button>
          </div>
          <p className="text-[10px] text-[var(--text-secondary)] mt-2 text-center">Don't worry about perfect wording. Explain it the way YOU understand it.</p>
        </div>
      </main>
    )
  }
        ...
        {evaluating ? (
          ...
          <>
            ...
            <div className="glass rounded-2xl p-6 mb-6 text-center">
              <div className="text-4xl font-bold" style={{ color: evaluation.overallScore >= 70 ? 'var(--success)' : evaluation.overallScore >= 40 ? 'var(--warning)' : 'var(--error)' }}>{evaluation.overallScore}%</div>
            ...
            {evaluation.understood.length > 0 && (
              <div className="glass rounded-2xl p-5 mb-4">
                ...
                <div className="flex flex-wrap gap-2">{evaluation.understood.map((item, i) => <span key={i} className="px-3 py-1.5 rounded-lg text-xs" style={{ background: 'rgba(52, 211, 153, 0.12)', color: '#34d399' }}>{item}</span>)}</div>
              </div>
            )}
            {evaluation.struggled.length > 0 && (
              <div className="glass rounded-2xl p-5 mb-4">
                ...
                <div className="flex flex-wrap gap-2">{evaluation.struggled.map((item, i) => <span key={i} className="px-3 py-1.5 rounded-lg text-xs" style={{ background: 'rgba(251, 191, 36, 0.12)', color: '#fbbf24' }}>{item}</span>)}</div>
              </div>
            )}
            {evaluation.weakSpots.length > 0 && (
              <div className="glass rounded-2xl p-5 mb-4">
                ...
                <ul className="space-y-2">{evaluation.weakSpots.map((spot, i) => <li key={i} className="text-xs flex items-start gap-2"><span className="text-[var(--error)] mt-0.5">•</span><span>{spot}</span></li>)}</ul>
              </div>
            )}
            {evaluation.misconceptions.length > 0 && (
              <div className="glass rounded-2xl p-5 mb-4">
                ...
                <div className="space-y-3">{evaluation.misconceptions.map((mc, i) => <div key={i} className="p-3 rounded-xl" style={{ background: 'rgba(248, 113, 113, 0.08)' }}><div className="text-xs font-semibold text-[var(--error)] mb-1">{mc.topic}</div><div className="text-[11px] mb-1"><span className="opacity-60">You said:</span> {mc.studentSaid}</div><div className="text-[11px]" style={{ color: 'var(--success)' }}>✓ {mc.correction}</div></div>)}</div>
              </div>
            )}
            {evaluation.suggestions.length > 0 && (
              <div className="glass rounded-2xl p-5 mb-8">
                <h3 className="text-xs font-semibold text-[var(--accent)] uppercase tracking-wider mb-3">💡 What to do next</h3>
                <ul className="space-y-2">{evaluation.suggestions.map((s, i) => <li key={i} className="text-xs flex items-start gap-2"><span className="text-[var(--accent)] mt-0.5">→</span><span>{s}</span></li>)}</ul>
              </div>
            )}
            <div className="flex gap-3">
              <button onClick={resetSession} className="flex-1 py-3.5 rounded-xl text-sm font-medium transition-all" style={{ background: 'var(--bg-card)', color: 'var(--text)', border: '1px solid var(--border)' }}>Study Something Else</button>
              <button onClick={continueTeaching} className="flex-1 py-3.5 rounded-xl text-sm font-medium text-white transition-all" style={{ background: 'linear-gradient(135deg, #6366f1, #818cf8)', boxShadow: '0 4px 20px rgba(99, 102, 241, 0.25)' }}>Keep Teaching</button>
            </div>
          </>
        ) : (
          <div className="text-center py-16">
            ...
            <button onClick={resetSession} className="px-6 py-3 rounded-xl text-sm font-medium text-white" style={{ background: 'linear-gradient(135deg, #6366f1, #818cf8)' }}>Start Over</button>
          </div>
        )}
      </div>
    </main>
